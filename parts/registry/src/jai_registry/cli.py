"""jai-registry CLI: sync the fleet from disk, then show the roster.

    jai-registry sync   # load parts/agents/*/manifest.yaml + evals/results/*.json
...
from jai_registry.store import AgentRecord, Registry
...
# parts/registry/src/jai_registry/cli.py -> repo root is four parents up.
...
def _print_table(records: list[AgentRecord]) -> None:
    if not records:
        print("(roster is empty — run `jai-registry sync`)")
        return
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="jai-registry", description="the fleet roster")
